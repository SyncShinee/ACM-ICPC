...
using namespace std;
const int N = 512, M = 50010, INF = 1e9 + 10;
struct edge {
    int to, nxt, val;
}e[N << 1];
struct Edge {
...
int head[N], n, m, side, MST, f[N], Rank[N], pos;
...
int deep[N], fa[N], subtree_size[N], son[N], top[N], edge_p[N], fedge_p[N], ans1, ans2;
...
void pushDown(int rt) {
    if (seg[rt].tag != -1) {
        seg[rt << 1].tag = seg[rt << 1].val = seg[rt].tag;
        seg[rt << 1 | 1].tag = seg[rt << 1 | 1].val = seg[rt].tag;
        seg[rt].tag = -1;
    }
}
